...
dt = 3600*24*ndays
surf = 300 #km2 GRANDproto300
#surf = 200000 #km2 GRAND
...
for i in range(np.size(eu)):
  ...
  for t in range(np.size(th)):
    ...
    sphi = sn[sel[0][0]]*surf*4  # GRAND physical area projected along traj  [km2]. Factor 4 comes from try area = 0.25km^2.
...
sefftopt = sefftopt*1e6  #m2dt = 3600*24*ndays  #1 day
print ee
...
# HE events
e2 = [pow(10,float(x)/10) for x in range(180,200)]
f2 =[J2*pow(x,-gamma2) for x in e2]
evt1doptHE = sefftopt[0]*dt*np.trapz(f2,e2)
print 'Optimal daily event rate in 10^18-10^20eV:',evt1doptHE
