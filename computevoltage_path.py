...
if __name__ == '__main__':
  ...
  if len(sys.argv)<5:
        print 'Wrong number of arguments. Usage: python computevoltage.py [zenith] [azimuth] [slope] [path to traces] [path to voltages] [effective 1/0]  [opt: AntennaID] [opt: antenna x,y,z]'
  ...
  path=sys.argv[4] #folder containing the traces
  pathout=sys.argv[5] #folder where the output should go to
  ...
  for l in range(start,end):
    ...
    # Compute effective zenith
            ...
            if len(sys.argv)==11:
                    ...
                    x_sim = float(sys.argv[8])
                    y_sim = float(sys.argv[9])
                    z_sim = float(sys.argv[10])
