...
#wkdir = '/home/martineau/GRAND/soft/old/neutrinos/simulations/'
wkdir = '/home/olivier/GRAND/data/GRAND/antennaModel/'
import linecache
from scipy.fftpack import rfft, irfft, rfftfreq
from scipy.interpolate import interp1d
sys.path.append('/home/martineau/GRAND/soft/old/neutrinos/retro/lib/python')  # To be able to import retro
#import retro
...
#===========================================================================================================
    ...
    plt.figure()
    plt.subplot(211)
    plt.plot(F/1e6,modulust)
    plt.xlabel('Frequency (MHz)')
    plt.ylabel('|Leff theta| (m)')
    plt.subplot(212)
    plt.plot(F/1e6,modulusp)
    plt.xlabel('Frequency (MHz)')
...
#===========================================================================================================
  ...
  for event in EventIterator(json_file):
    ...
    #	return
    print("This is now OBSOLETE in this version. Check previous ones")
    ### DECAY
    ...
    #else: # pion-like
    #    primary="pion"
    # Hack for GP300 APril 18, 2019
    primary="pion"

    return zenith_sim,azimuth_sim,energy,injection_height,primary,decay_pos
...
#===========================================================================================================
#def compute(opt_input,path, effective,zenith_sim, azimuth_sim, energy, injection_height, primary,json_file,decay_pos):
def compute(opt_input,path, effective,zenith_sim, azimuth_sim, energy, injection_height, primary,json_file):
#===========================================================================================================
    ...
    # Compute Xmax
    #'''This is now directly read from the .sry file'''
    ...
    #Xmax = decay_pos+Xmax_distance*np.array([caz*szen, saz*szen, czen])
    #print [0,0,injection_height],Xmax_distance*np.array([caz*szen, saz*szen, czen]),Xmax
    #print('Injection point=',injection_height)
    #print('Xmax=',Xmax_primary,'g/cm2, Xmax height=',Xmax_height,'m asl, Xmax distance to ground=',Xmax_distance,'m, Xmax position= ',Xmax)
    ...
    if len(sryfile) == 0:
        print("Error! sry file is missing in folder",path,". Abort.")
        return
    with open(sryfile[0]) as f:
      for line in f:
        if 'Location of max' in line:
            a = line.split(':')[-1].split()
            Xmax = np.array([float(a[2]),float(a[3]),float(a[4])])*1e3 #m
            Xmax_distance = float(a[0]) #m
            Xmax_height = float(a[1]) #m
        if 'Sl. depth of max.' in line:
            a = line.split(':')[-1].split()
            Xmax_primary = float(a[0]) #g/cm2
    ...
    for l in range(start,end):
        ...
        print('Wave direction: zenith = ', zenith_sim, ' deg, azimuth = ', azimuth_sim, 'deg. (GRAND conventions), mountain slope: ', 0, 'deg.')
        print('Efield file: ', efieldtxt)

        # Model the input signal.
        try:
            time1_sim, Ex_sim, Ey_sim,Ez_sim = np.genfromtxt(efieldtxt,usecols=(0,1,2,3),unpack=True)
        except IOError:
            continue
        ...
        if (opt_input=='json' or opt_input=='txt') and (len(sys.argv)==11) :
            ...
            try :
                if opt_input=='json':
                    ...
                    #[x_sim, y_sim, z_sim, alpha_sim, beta_sim] = map(float, line.split(" ")) # THis is not the way it is in this file
                    [id, name, x_sim, y_sim, z_sim] = map(str, line.split())
	...
	#Xant = [x_sim, y_sim, 0]
 	#Xant = [40000,0 , 0]
        ...
        if np.size(timeEW)>0:   # Dat was computed
          if freespace == 1:
            f = open(path+'/out_'+str(l)+'_0.txt',"w")
          else:
            f = open(path+'/out_'+str(l)+'.txt',"w")
          print('OUTFILE : ', f)
          #Python2 syntax
...
if __name__ == '__main__':
	...
	Wrong minimum number of arguments. All angles are to be expressed in degrees and in GRAND convention.
        Usage:
        if ZHAireS inp file input:
            python computevoltage.py txt 1 [path to traces] [inp file]
            example: python computeVoltage_CRs.py txt 0 ~/GRAND/soft/grand/tests/simulation/data/zhaires/ ~/GRAND/soft/grand/tests/simulation/data/zhaires/Stshp_125m_0.1995_85.220_0_Proton_08.inp

        if json  file input:
            python computevoltage.py json [effective 0/1] [path to traces] [json file] [opt: antenna ID] [opt: antenna x,y,z,alpha,beta]
    ...
    if opt_input=='txt':
        ...
        zenith_sim,azimuth_sim,energy,injection_height,primary,decay_pos = inputfromjson(path,json_file)
